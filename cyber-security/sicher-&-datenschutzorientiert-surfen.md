...
`Anleitung erstellt am 10.2.2025, zuletzt bearbeitet am 23.02.2026`
...
2. Irrglaube [VPN](https://de.wikipedia.org/wiki/Virtual_Private_Network)-Dienste
    - eigene/ selbstgehostete [VPN](https://de.wikipedia.org/wiki/Virtual_Private_Network) Server
...
- Es ist jedoch wichtig, zwischen VPN-Diensten durch kommerzielle Anbieter von VPNs die häufig von Unternehmen eingesetzt werden, um Clients mit dem Unternehmensnetzwerk zu verbinden, oder privaten VPNs von Privatpersonen in das eigene Heimnetz, um z.B. Dienste aus dem Heimnetz zu nutzen, zu unterscheiden.
...
- Nachteile von VPN-Diensten könnten z.B. sein:
	...
	- kein DNS-leak Schutz: Manche Anbieter haben keinen DNS-Leak-Schutz, der Verhindert, dass DNS-Anfragen zu lokalen DNS-Servern/ DNS-Servern des ISPs gelangen, wodurch die wahre IP-Adresse geleakt werden kann
	- VPN-Anbieter kennt Ziel- und Quell-IP-Adressen
	- ein Großteil des sensiblen Netzwerkverkehrs wird bereits mit TLS verschlüsselt, z.B. Standard für Webseiten ist HTTPS (HTTP-over-TLS-Verschlüsselung)
	- VPN-Anbiter könnten Nutzerdaten/Netzwerkverkehr an Strafverfolgungsbehörden weitergeben
	- mangelnde/ schlechte Datenschutzgesetze im Land mit dem Hauptsitz/ Ländern mit Serverstandorten des Anbieters
...
### eigene/ selbstgehostete [VPN](https://de.wikipedia.org/wiki/Virtual_Private_Network) Server
- Eine immer beliebtere Methode sind eigene VPN-Server im Heimnetz, also zuhause. Mit einer aktuellen FritzBox ist das sogar für wenig technisch versierte Nutzer möglich. Dieser VPN kann sowohl für Desktop-Clients (Windows, MacOS, Linux) als auch auf Android/iOS eingerichtet werden.
- Alternativ ist es natürlich auch möglich, auf einem extern angemieteten Server in einem Rechenzentrum, einen eigenen [VPN-](https://de.wikipedia.org/wiki/Virtual_Private_Network)/[WireGuard](https://www.wireguard.com/)-Server zu installieren, um den Netzwerkverkehr von mobilen Geräten, als auch von Heimnetz aus über einen solchen Server laufen zu lassen, um Tracking durch Internetprovider etc. zu vermeiden. Auch Schutz vor Tracking durch Webseiten-Betreiber über die IP-Adresse ist in diesem Szenario möglich.
...
- Vorteile:
	...
	- Netzwerkverkehr vor Tracking/Logging durch Mobilfunk-/Internetanbietern (ISPs) in mobilen Netzwerken, fremden Netzwerken oder öffentlichen WLANs schützen.
	...
	- Teilweise Umgehung von Geoblocking, z.B. wenn sich Client im z.B. Ausland befindet und VPN-Server im Heimnetz z.B. in Deutschland ist.
	- Sicheren Zugang zum Heimnetz, z.B. (wenn vorhanden) Zugriff auf Homeserver/[NAS](https://de.wikipedia.org/wiki/Network_Attached_Storage),[Nextcloud](https://nextcloud.com/de/) etc.
...
- Browser-Erweiterungen
	...
	- Es empfiehlt sich grundsätzlich, `nur` die `nötigsten` Erweiterungen installiert zu haben, da mehr Software potentiell auch mehr Sicherheitslücken mit sich bringen kann.
...
- [Privacy Badger](https://privacybadger.org/)
	...
	- Dabei "lernt" der Privacy Badger und blockiert vor allem Tracker, die er auf mehreren Seiten erkennt.
...
- Was ist Fingerprinting ?
	...
	- Das Ziel des Fingerprintings ist es, eindeutige Nutzerprofile zu erstellen und über Webseiten hinweg, Nutzer zu verfolgen und zu identifizieren.
	- Das Ganze geschieht `ohne` Cookies und `ohne Zustimmung` des Nutzers und wird durch Tracker, die auf vielen verschiedenen Webseiten platziert sind, durchgeführt.
...
- Wie kann man Fingerprinting verhindern/blockieren ?
	...
	- Außerdem sollte man sowieso den Browser immer auf `Updates` prüfen, `nur Standardschriftarten` verwenden, sowie `Englisch als Sprache im Browser` auswählen.
...
- [Brave-Browser](https://brave.com/de/)
    ...
    - Alltagstauglich: Ausgewogener Browser zwischen Sicherheit/Datenschutz und Alltagstauglichkeit !
...
- [Mullvad Browser](https://mullvad.net/en/browser)
	...
	- Sehr starker Fokus auf Anonymität und Privatspähre, jedoch NICHT für jeden Nutzer "alltagstauglich" !
...
- [Firefox](https://www.firefox.com/de/)
	...
	- Tracking-Schutz vorhanden, aber nicht so gut wie bei Brave oder Mullvad
	- Fingerprinting-Schutz ist schwach, muss ggf. mit Erweiterungen ergänzt werden
	- gute Alternative und Gegenpol zum Google-Monopol mit den Chromium-basierten Browsern
	- Alltagstauglich, jedoch standardmäßig Tracking/Diagnose aktiv und Tracking/Fingerprinting-Schutz muss manuell eingestellt werden !
...
- [Vivaldi](https://vivaldi.com/de/)
	...
	- Außerdem werden Werbeanzeigen von Werbepartnern (von Vivaldi) standardmäßig nicht blockiert, das kann man jedoch deaktivieren. Dafür in den Einstellungen des Browsers unter "Tracker- und Werbeblocker" > "Werbeblocker-Quellen" > "Werbung von unseren Partnern erlauben (unterstützen Sie Vivaldi)" ausschalten.
...
- `WARNUNG vor den Opera-Browsern:`
    ...
    - Opera ist ebenfalls voll mit externen Trackern, von z.B. Microsoft, Google und Facebook.
...
- [Startpage](https://www.startpage.com/)
	- kein Tracking
	- keine Speicherung von personenbezogenen Daten, IP-Adressen oder Suchanfragen
	- keine personalisierte Werbung (Werbung basiert auf Suchkategorie)
	- Startpage sammelt Informationen bezüglich der Sprache, des Betriebssystems und des User-Agents des Browsers
	- Quelle der Suchergebnisse: Google, teilweise Bing (anonymisiert)
    - Startpage gehört einem US-Amerikanischen Unternehmen (System1), welches die Mehrheit an Startpage besitzt (Stand 2026).
    - System1 ist im Bereich Werbetechnologie tätig, jedoch hat Startpage selber seine eigene Datenschutzrichtlinie unabhänig von System1.
    - Seit 2024 hat Startpage frühere Einschränkungen bei der Nutzung von VPNs und Tor entfernt.
	- Niederländisches Unternehmen
	- Fazit: sehr gute Suchergebnisse, Suchanfragen werden anonymisiert, (EU-)Datenschutz, ohne Tracking & keine personalisierte Werbung


- [Qwant](https://www.qwant.com/)
	- kein Tracking
	- keine Speicherung von personenbezogenen Daten oder IP-Adressen
	- keine personalisierte Werbung
	- Quelle der Suchergebnisse: Bing (anonymisiert)
	- teilweise bei Such- und Werbeauslieferung von Microsoft abhängig
	- teilweise Geräte-/Browserdaten & IP-bezogene Daten werden an Microsoft für Such-/ Werbeauslieferung übermittelt
	- Französisches Unternehmen
	- Fazit: Für Europäische Nutzer, (EU-)Datenschutz, ohne Tracking, jedoch abhängig von Microsoft
...
- [DuckDuckGo](https://duckduckgo.com/)
	...
	- keine Speicherung von personenbezogenen Daten oder IP-Adressen
	- keine personalisierte Werbung (Werbung basiert auf Suchkategorie)
	- DuckDuckGo speichert jedoch Suchanfragen für "Produktverbesserungen"
	- Quelle der Suchergebnisse: teilweise eigener Index & Bing, für "instant-Answers" werden auch z.B. Wikipedia oder Tripadvisor, Yandex oder Yahoo verwendet
    - Server werden nicht selbst betrieben, sondern bei externem Anbieter (in Amerika) angemietet
    - standardsuchmaschine im Tor-Browser
...
- [Ecosia](https://www.ecosia.org/)
    ...
    - Suchanfragen werden teilweise mit externen Partnern verarbeitet, dabei werden ggf. Suchbegriffe und IP-Adressen nach Ecosia-seitiger "anonymisierung" übermittelt
...
- [Google](https://www.google.com/)
	...
	- püersonalisierte Werbung
	- Quelle der Suchergebnisse: Eigener Index
	- Amerikanisches Unternehmen
	- Fazit: weit verbreitete Suchmaschine, viel Tracking, Logging und personalisierte Werbung
...
- [DNS](https://de.wikipedia.org/wiki/Domain_Name_System)-Leaks im Browser
    ...
    - Die Ursachen von DNS-Leaks sind in der Regel falsch konfigurierte VPNs oder VPN-Anbieter leiten DNS-Anfragen nicht über die VPN-Verbindung.
...
- Der Tor-Browser hat viele gute und sinnvolle Einsatzzwecke. Für den durchschnittlichen Nutzer im Alltag ist er jedoch ungeeignet, da das Tor-Netzwerk leider recht langsam ist und durch einige Sicherheitseinstellungen und das Blockieren von JavaScript, ist Tor-Browser für den Alltag eher unbrauchbar.
- Bei der Browserwahl muss jedoch jeder individuell seinen Kompromiss zwischen Sicherheit, Performance und Benutzerfreundlichkeit finden.

### Anonymität durch Tor ?
- `ACHTUNG`, die Verwendung des [Tor-Browsers](https://www.torproject.org/) bedeutet niemals absolute "Anonymität", allerdings gilt der Tor-Browser vergleichsweise als der sicherste Browser.
...
# 8. Fazit - Checkliste Datenschutz-/Sicherheitseinstellungen
- Abschließend kann man festhalten, dass anonym zu surfen ein Konzept ist, man sich jedoch datenschutzorientiert und sicherheitsbewusst für bestimmte Einstellungen oder Dienste entscheiden, bzw. bewusst gegen die Nutzung von bestimmten Einstellungen oder Diensten entscheiden kann.
...
- `Wissen und das Auseinandersetzen mit diesem Thema sind der beste "Schutz".`
...
- Verwendung einer datenschutzorientierten Suchmaschine ?
- Browsereinstellungen überprüft ?
- Spracheinstellungen auf "Englisch" gesetzt ?
- [WebRTC](https://en.wikipedia.org/wiki/WebRTC) ggf. deaktiviert/ Leak-Schutz aktiviert ?
- [DNS](https://de.wikipedia.org/wiki/Domain_Name_System)-Anfragen verschlüsselt (im Browser/Betriebssystem oder Router) ?
...
- [european-alternatives.cloud](https://european-alternatives.cloud/de/)
    - [european-alternatives.cloud/browser](https://european-alternatives.cloud/de/browse?category=browser)
    - [european-alternatives.cloud/suchmaschine](https://european-alternatives.cloud/de/browse?category=search-engine)
...
- [The Morpheus](https://www.youtube.com/@TheMorpheusVlogs) - Tech & Software / CyberSecurity & AI - YouTuber
	...
	- [Startpage: detaillierte Analyse mit Insider Infos](https://youtu.be/DgCXWoByPk4?si=6WDECbPXbVT4xfN3)
	- [ENGLISCH - Interview mit Robert Beens, CEO von Startpage](https://youtu.be/z3RdoZKFuMs?si=mqJMpqgO51e6KpNw)
